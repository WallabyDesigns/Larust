...
/// rendered `<div data-live-id="...">` wrapper — what a `@live(...)`
/// codegen call site splices directly into the surrounding page's HTML.
///
/// Every full-page GET through a `@live(...)` mount point creates a
    ...
    with_session_lock(session, || async {
        let entry = registry::lookup(name).ok_or_else(|| {
            AppError::Internal(Box::new(std::io::Error::other(format!(
                "@live('{name}', ...) used but no component is registered under that name — \
                 call `larust_support::live::components().register::<YourType>().publish()` \
...
/// Wraps a component's rendered output in the `data-live-id`-carrying
/// `<div>` the client runtime addresses `/__larust_live/{id}` through — the
...
/// No `data-live-name` in the markup — the server resolves id → name from
/// session storage; the client only ever needs the opaque id.
pub(crate) fn wrap(id: &str, inner_html: &str) -> String {
    format!(r#"<div data-live-id="{}">{inner_html}</div>"#, escape(id))
}
