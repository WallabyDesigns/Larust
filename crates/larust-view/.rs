...
fn collect_pushes(
    ...
    for node in nodes {
        match node {
            ...
            Node::Section { body, .. } => collect_pushes(body, pushes)?,
            _ => {}
        }
    }
    Ok(())
...
fn contains_push(nodes: &[Node]) -> bool {
    nodes.iter().any(|n| match n {
        ...
        Node::Foreach { body, .. } | Node::Section { body, .. } => contains_push(body),
        _ => false,
    })
}

/// Recursively finds every `@globals` block in `nodes` (including inside
/// `@if`/`@foreach`/`@section`/`@push` bodies) and merges its `name = expr`
/// entries into `globals`.
///
    ...
    for node in nodes {
        match node {
            ...
            Node::If {
                ...
                collect_globals_into(then_branch, local)?;
                collect_globals_into(else_branch, local)?;
            }
            Node::Foreach { body, .. } => {
            ...
            Node::Section { body, .. } | Node::Push { body, .. } => {
                collect_globals_into(body, local)?;
            }
            _ => {}
        }
...
/// `@if`/`@foreach`/`@section`/`@push` — used only to produce
/// `collect_globals`'s "`@globals` inside `@foreach`" error eagerly, before
/// walking (and partially registering) the offending subtree.
fn contains_globals(nodes: &[Node]) -> bool {
    nodes.iter().any(|n| match n {
        ...
        Node::Foreach { body, .. } | Node::Section { body, .. } | Node::Push { body, .. } => {
            contains_globals(body)
        }
        _ => false,
    })
}
