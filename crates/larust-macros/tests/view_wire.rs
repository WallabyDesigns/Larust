//! End-to-end proof that `@wire(...)` works through the real `view!` macro
...
//! regression in `codegen_node`'s `Node::Wire` arm or the eager
...
async fn page(session: Session) -> Result<axum::response::Response, AppError> {
    let view = view!("wire_test", { session: &session });
