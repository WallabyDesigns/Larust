...
| `.env`'s `APP_URL` (a Laravel project's `.env` rarely sets `APP_PORT` at all) | `app_port` config resolution | Lenient by design: `APP_PORT` wins if set, otherwise a port parsed out of `APP_URL` (`http://127.0.0.1:8000` -> `8000`) is used before falling back to this framework's own generic default - see the [FAQ](../faq#why-port-34187-not-8000) |
